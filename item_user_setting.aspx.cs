...
public partial class item_user_setting : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            getgroup();
            showTempData();//會出現不可以在 DropDownList 中選取多個項目
            d_search();
    ...
    private DataTable showdata(string sortExpression = null, string sortDirection = "ASC")
    {
        using (SqlConnection cn = new SqlConnection(eip))
        {
            ...
            string sql = @"SELECT  [user_id],[sn],t1.[name],t3.name as gid,t2.name as user_group ,[job],[user_right_id],[note],[state] FROM [eip_user] as t1
                    left join group_name as t2 on t1.user_group=t2.id
					left join group_name as t3 on t3.id=t1.gid
                    where user_right_id=1 ";
    ...
    protected DataTable search(string sortExpression = null, string sortDirection = "ASC")
    {
        ...
        using (SqlConnection cn = new SqlConnection(eip))
        {
            ...
            //{
            //    sql += " and user_right_id=1";
            //}
            //else if (search1_rb_up2.Checked)
            //{
            //    sql += " and user_right_id=3";
            //}
            
            //else if (search1_rb_down.Checked)
            //{
            //    sql += " and user_right_id=0";
            //}
    ...
    protected void searchtb()
    {
        ...
        using (SqlConnection cn = new SqlConnection(eip))
        {
            ...
            //        left join group_name as t2 on t1.user_group=t2.id
            //        where 1=1 ";
            sql = @"SELECT  [user_id],[sn],t1.[name],t3.name as gid,t2.name as user_group ,[job],[user_right_id],[note],[state],specil_show_page FROM [eip_user] as t1
                    left join group_name as t2 on t1.user_group=t2.id
					LEFT JOIN group_name AS t3  ON t3.id = ISNULL(t2.parent_id, t1.user_group)
                    where 1=1 ";
            ...
            {
                sql += " AND t3.id = @gid";
                SqlDataSource1.SelectParameters.Add("gid", group.SelectedValue);
            }
            ...
            {
                sql += " and specil_show_page=1";
            }
            else if (rb_status3.Checked)
            {
                sql += " and specil_show_page=2";
            }
            else if (rb_status4.Checked)
            {
                sql += " and specil_show_page=3";
            }
            else if (rb_status5.Checked)
            {
                sql += " and specil_show_page=4";
            }
            else if (rb_status6.Checked)
            {
                sql += " and specil_show_page=5";
            }
            else if (rb_status7.Checked)
            {
                sql += " and specil_show_page=6";
            }
            else if (rb_status8.Checked)
            {
                sql += " and specil_show_page=7";
            }
            else if (rb_status9.Checked)
            {
                sql += " and specil_show_page=8";
            }
            else if (rb_status0.Checked)
            {
                sql += " and specil_show_page=0";
            }
            if (keyword.Text != "")
            {
    ...
    protected void gv_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            string ShowPage = DataBinder.Eval(e.Row.DataItem, "specil_show_page").ToString();
    ...
    protected void gv_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "modify")
        {
            try
            {
                ...
                string url = string.Format("item_user_edit.aspx?m=1&id={0}",
    ...
    protected void showTempData()
    {
        ...
        //{
        //    group.SelectedValue = Session["l_temp_group2"].ToString();//會出現不可以在 DropDownList 中選取多個項目
        //}
    ...
    //{
    //    string url = string.Format("item_user_edit.aspx?m=2");
    ...
    protected void listBtn_Click(object sender, EventArgs e)//人員清單下載
    {
        ...
        using (SqlConnection cn = new SqlConnection(eip))
        {
            ...
            if (group.SelectedValue != "0")
            {
                cmd.Parameters.AddWithValue("@gid", group.SelectedValue);
            }
            if (add_unit_class.SelectedValue != "0")
            {
            ...
            while (dr.Read())
            {
                ...
                ManageState.Add(dr["specil_show_page"].ToString());
        ...
        Response.AddHeader("Content-Disposition", "attachment; filename=特殊用品系統人員清單" + dateTime.ToString("yyyyMMddHHmmss") + ".xls");
