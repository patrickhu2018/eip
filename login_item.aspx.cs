...
public partial class login_item : System.Web.UI.Page
{
                ...
                if (isValidSession)
                {
                    ...
                    using (SqlConnection cn = new SqlConnection(eip))
                    {
                        ...
                        string sql = @"select user_id,t1.name,specil_user_right_id,user_group,t2.name as group_name,t1.job,HomePage from [eip_user] as t1
                                   ...
                                   where sn=@sn and user_group like @user_group order by user_id desc";

                        //where  user_group=@user_group and t1.name=@name and t1.sn=@sn
